...
function completeOptions(opts){
    ...
    opts = changing({
        logoutRedirect    : opts.noLoggedUrlPath
    },opts);
    return opts;
}
...
loginPlus.Manager.prototype.init = function init(app,opts){ // eslint-disable-line max-statements
    var thisManager = this;
    if(!this.passport){
        var router = '_router' in app ? app._router : app.router;
        if(router){
            var forbidensMiddlewares=router.stack.filter(function(layer){
            ...
            if(forbidensMiddlewares.length){
                ...
                throw new Error('loginPlus unexpected router. Check that cookie-parse was set with initCookieParse');
            }
        }
        SetInitialOptions.call(this,app,opts);
    }
    opts = this.opts;
    this.ensureMiddleware = ensureLoggedIn({baseUrl: opts.baseUrl, redirectTo:opts.noLoggedUrlPath, setReturnTo:Regexplicit.detectNonEmptyUrlWithoutResources});
    ...
    app.get(opts.baseUrl+opts.logoutUrlPath, function(req,res,next){
        req.logout(function(err) {
            ...
            res.redirect(opts.redirBaseUrl+opts.logoutRedirect);
        });
    });
    /*
    app.post(opts.baseUrl+opts.loginUrlPath, function(req,res,next){
        if(req.query["return-to"] && req.session){
            console.log('XXXXXX- CHECK LOGIN PLUS', req.query["return-to"]);
            req.session.returnTo = req.query["return-to"];
        }
        next();
    });
    */
    ...
    app.post(opts.baseUrl+opts.chPassUrlPath, function(req,res){
        if (!req.body.newPassword) {
            req.flash('error', (opts.messages?.passCantBeEmpty || "new pass cant be empty"));
            res.redirect(opts.redirBaseUrl+opts.chPassUrlPath)
        }else if (req.body.newPassword !== req.body.repPassword) {
            req.flash('error', (opts.messages?.passDoesntMatch || "new pass doesn't match"));
            res.redirect(opts.redirBaseUrl+opts.chPassUrlPath);
        }else if((req.body.newPassword||'').length>=opts.passMinLength){
            thisManager.passwordChanger(
                req,
                req.user[opts.userFieldName],
                req.body.oldPassword,
                req.body.newPassword,
                function(err, ok, flashMessage){
                    if(ok){
                        req.logout(function(err) {
                            if (err) { return next(err); }
                            req.session.returnTo = null;
                        ...
                        req.flash('error', flashMessage || opts.messages?.internalError || 'internal error');
                        console.log('probablemente hay un error en los campos de la tabla usuarios (quizas no coincidan con los esperados)')
                        res.redirect(opts.redirBaseUrl+opts.chPassUrlPath);
                    }
                }
            );
        }else{
            req.flash('error', (opts.messages?.passTooShort || 'pass too short (minimum length $1)').replace('$1',opts.passMinLength));
            res.redirect(opts.redirBaseUrl+opts.chPassUrlPath);
        }
    });
    return promiseChain;
