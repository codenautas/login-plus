...
loginPlus.Manager.prototype.init = function init(app,opts){
    ...
    opts = this.opts;
    this.ensureMiddleware = ensureLoggedIn({baseUrl: opts.baseUrl, setReturnTo:/^([^/]*|.*\/)[^.]+$/});
    ...
    app.get(opts.baseUrl+opts.logoutUrlPath, function(req,res,next){
        req.logout();
        req.session.thePreviousUrl = null;
    ...
    app.post(opts.baseUrl+opts.loginUrlPath,
        ...
        function(req,res){
            res.redirect(req.session.thePreviousUrl?req.session.thePreviousUrl:opts.redirBaseUrl+opts.successRedirect);
        }
    );
        ...
        if(req.body.newPassword.length>=opts.passMinLength){
            thisManager.passwordChanger(
                ...
                function(err, ok, flash){
                    if(ok){
                        req.logout();
                        req.session.thePreviousUrl = req.headers.referer;
                        res.redirect(opts.redirBaseUrl+opts.loginUrlPath);
                    }else{
